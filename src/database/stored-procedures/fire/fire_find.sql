DROP PROCEDURE IF EXISTS fire_find;
CREATE PROCEDURE fire_find(
...
BEGIN
    ...
    IF code_val <> '' THEN
        SET code_cond = CONCAT(' AND f.fireCode LIKE "%', code_val, '%"');
    END IF;
    IF name_val <> '' THEN
    ...
    SET @final_query = CONCAT(
            'INSERT INTO tmp_data (id, code, name, number,city,district,province,area)
             SELECT id,fireCode,fireName,contactNumber,city,district,province,areaCovered
             FROM tbl_fire f
             WHERE f.id > 0', name_cond, area_cond,
