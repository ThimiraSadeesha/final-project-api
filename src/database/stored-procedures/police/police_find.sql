DROP PROCEDURE IF EXISTS police_find;
CREATE PROCEDURE police_find(
...
BEGIN
    ...
    IF code_val <> '' THEN
        SET code_cond = CONCAT(' AND p.policeCode LIKE "%', code_val, '%"');
    END IF;
    IF name_val <> '' THEN
        SET name_cond = CONCAT(' AND p.policeName LIKE "%', name_val, '%"');
    END IF;
    ...
    SET @countQuery = CONCAT(
            'INSERT INTO tmp_count (total_count)
             SELECT COUNT(*)
             FROM tbl_police p
             WHERE p.id > 0', name_cond, area_cond, ';'
                      );
    ...
    SET @final_query = CONCAT(
            'INSERT INTO tmp_data (id, code, name, number,city,district,province,area)
             SELECT id,policeCode,policeName,contactNumber,city,district,province,areaCovered
             FROM tbl_police p
             WHERE p.id > 0', name_cond, area_cond,
            ' GROUP BY p.id ORDER BY p.id DESC', limit_cond, ';'
                       );
